//
//  UMAuditForeignAirordercostViewController.m
//  vlogmb
//
...
#import "UMAuditForeignAirordercostViewController.h"
#import "ASFTableView.h"
#import "UMAppDelegate.h"
#import "AFNetworking.h"
#import "DXAlertView.h"
@interface UMAuditForeignAirordercostViewController ()
@property (nonatomic, retain) NSMutableArray *rowsArray;
@end

@implementation UMAuditForeignAirordercostViewController
...
- (void)viewDidLoad
{
    ...
    [self listForeignByWorkno];
}
-(void) listForeignByWorkno
{
    
    NSString *string=[[[UMAppDelegate basePath] stringByAppendingString:@"airorderCostAction!listForeignByWorkno.action?workno="] stringByAppendingString:self.workno];
    ...
    [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSArray *weights = @[@(0.12f),@(0.32f),@(0.36f),@(0.09f),@(0.13f)];
...
- (IBAction)pass:(id)sender {
    ...
    alert.leftBlock = ^() {
        ...
        NSString *string=[[[[[UMAppDelegate basePath] stringByAppendingString:@"virtualTaskAction!tagAuditForeignAirorderCostFinished.action?workno="] stringByAppendingString:self.workno] stringByAppendingString:@"&accountid="] stringByAppendingString:[accountid stringValue]];
        NSURL *url=[NSURL URLWithString:string];
        NSURLRequest *request=[NSURLRequest requestWithURL:url];
...
- (IBAction)refuse:(id)sender {
    ...
    [alert show];
    alert.leftBlock = ^() {
        NSString *string=[[[[[UMAppDelegate basePath] stringByAppendingString:@"virtualTaskAction!tagAuditForeignAirorderCostRefused.action?workno="] stringByAppendingString:self.workno] stringByAppendingString:@"&opinion="] stringByAppendingString:@"wrong cost"];
